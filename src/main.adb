...
procedure Main is
   ...
   begin
      ...
      vec2:=(-4,-1);
      get(vec1,3,bool);
      Assert_True (bool and then vec1(3)=0, Msg & " get succesfuly in the middle");
      get(vec1,1,bool);
      Assert_True (bool and then vec1(1)=0, Msg & " get succesfuly in the First element");
      get(vec1,5,bool);
      Assert_True (bool and then vec1(5)=0, Msg & " get succesfuly in the last element");
      get(vec1,10,bool);
      Assert_True (bool = False, Msg & " get fail");
      get(vec1,2,bool);
      Assert_True (bool and then vec1(2)=0, Msg & " get succesfuly");
      get(vec2,4,bool);
      Assert_True (bool = False, Msg & " get fail wiht one element");
      get(vec2,-4,bool);
      Assert_True (bool and then vec2(1)=0, Msg & " get sucesfuly wiht one element");
   exception
      when Assertion_Error =>
         Put_Line (Msg & " Failed (assertion)");
      when others =>
